...
class CodexAdapter(BaseProviderAdapter):
    ...
    def load_session(self, work_dir: Path, target: str | None = None) -> Optional[CodexProjectSession]:
        return load_project_session(work_dir, target)

    def compute_session_key(self, session: Any, target: str | None = None) -> str:
        return compute_session_key(session, target) if session else "codex:unknown"
