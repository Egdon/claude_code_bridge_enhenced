...
def load_project_session(work_dir: Path, target: str | None = None) -> Optional[GeminiProjectSession]:
...
def compute_session_key(session: GeminiProjectSession, target: str | None = None) -> str:
    ...
    if target_value:
        try:
            ...
            instance = "main"
    return f"gemini:{pid}:{instance}" if pid else "gemini:unknown"
