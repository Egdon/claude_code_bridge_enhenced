...
def _load_ccb_module() -> object:
    ...
    loader = SourceFileLoader("ccb_script_other_targets", str(ccb_path))
    spec = importlib.util.spec_from_loader("ccb_script_other_targets", loader)
...
@pytest.mark.parametrize(
    ...
    [
        ("gemini@2", "CCB-Gemini@2", ("instances", "gemini", "2")),
        ("opencode@2", "CCB-OpenCode@2", ("instances", "opencode", "2")),
        ("droid@2", "CCB-Droid@2", ("instances", "droid", "2")),
        ("cursor@2", "CCB-Cursor@2", ("instances", "cursor", "2")),
    ],
)
def test_start_target_non_codex_tmux_uses_target_runtime_and_registry(
    ...
    if target.startswith("cursor"):
        monkeypatch.setattr(launcher, "_get_latest_cursor_session_info", lambda: (None, None))
    ...
    assert launcher.tmux_panes[target] == "%21"
    assert target.split("@", 1)[0] not in launcher.tmux_panes
    assert title_calls == [('%21', pane_title)]
...
@pytest.mark.parametrize("target", ["gemini@2", "opencode@2", "droid@2", "cursor@2", "claude@2"])
def test_start_target_in_current_pane_routes_other_target(monkeypatch, tmp_path: Path, target: str) -> None:
    ccb = _load_ccb_module()
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".ccb").mkdir(parents=True, exist_ok=True)
    ...
    if target.startswith("gemini"):
        def _fake_gemini(value: str = "gemini@main") -> int:
            called["target"] = value
            return 0
        monkeypatch.setattr(launcher, "_start_gemini_current_pane", _fake_gemini)
    elif target.startswith("opencode"):
        def _fake_opencode(value: str = "opencode@main") -> int:
            called["target"] = value
            return 0
        monkeypatch.setattr(launcher, "_start_opencode_current_pane", _fake_opencode)
    elif target.startswith("droid"):
