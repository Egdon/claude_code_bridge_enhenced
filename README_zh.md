...
### 启动
```bash
ccb                                # 按 ccb.config 启动 target 列表
ccb codex@main claude@main         # 启动两个 target
ccb codex@1 codex@2 claude@main    # 两个 Codex + Claude anchor
ccb codex@1 codex@2 codex@3 codex@4 # 四个独立 Codex target
ccb codex@1,codex@2,claude@main    # 同上，逗号分隔
ccb -r codex@1 claude@main         # 恢复 target 会话
ccb -a codex@1 codex@2 claude@main # 自动权限模式
...
- target 语法是 `provider@instance`，启动时建议总是显式写 `@instance`。
- **最后一个 target 是 anchor**，会运行在当前 pane。
- 非 anchor target 的 pane 生成顺序可能和输入顺序不同，这是为了保持布局稳定。
- `ccb add` / `ccb rm` 优先走当前会话的 **live control plane**；如果 live socket 不可达，则 fallback 到持久化的 target session / registry。
- `tmux` 提示：CCB 的 tmux 状态栏/窗格标题主题只会在 CCB 运行期间启用。
- 布局规则：当前 pane 对应最后一个 target。额外 pane 顺序为 `[cmd?, targets[:-1] 反序]`；第一个额外 pane 在右上，其后先填满左列，再填右列。
- 详细说明见：[`TARGET_USAGE_GUIDE.zh.md`](TARGET_USAGE_GUIDE.zh.md)
...
兼容说明：旧的 provider-only 配置在迁移期内仍会被读取，并自动归一化成 `@main`。
...
### 旧命令（已废弃）
- `cask/gask/oask/dask/lask/uask` - 各 provider 独立的 ask 命令
- `cping/gping/oping/dping/lping/uping` - 各 provider 独立的 ping 命令  
- `cpend/gpend/opend/dpend/lpend/upend` - 各 provider 独立的 pend 命令

### 新统一命令
- **`ask <provider|provider@instance> <message>`** - 统一请求命令（默认后台）
  - 支持 provider: `gemini`, `codex`, `opencode`, `droid`, `claude`, `cursor`
  - 多实例场景建议显式写 target，例如 `ask codex@2 "..."`
  - 默认后台；可用 `--foreground` / `--background` 或 `CCB_ASK_FOREGROUND=1` / `CCB_ASK_BACKGROUND=1` 覆盖
  - 前台执行默认关闭 completion hook（除非设置 `CCB_COMPLETION_HOOK_ENABLED`）
  - 支持 `--notify` 用于短消息同步通知；当前必要时会回退到 legacy provider daemon 路径
  - 手工从外部 shell 调用时建议显式设置 `CCB_CALLER`，例如 `CCB_CALLER=claude ask codex@2 --foreground "..."`
  - 不建议在多实例场景里长期裸写 `ask codex ...`，因为它可能继承环境里的 target 推断

- **`cask/gask/oask/dask/lask/uask --target <provider@instance> <message>`** - provider 专用 ask 入口
  - 例：`cask --target codex@2 "..."`
  - 例：`lask --target claude@main "..."`
  - 例：`uask --target cursor@exp "..."`

- **`ccb-ping [target]` / `ccb-ping --provider <provider>` / `ccb-ping`** - 统一的连通性测试命令
  - 单 target：`ccb-ping codex@1`
  - 单 provider：`ccb-ping --provider codex`
  - 全部 target：`ccb-ping`

- **`ccb kill [target]` / `ccb kill --provider <provider>` / `ccb kill`** - 统一的终止 / 清理命令
  - 单 target：`ccb kill codex@2`
  - 单 provider：`ccb kill --provider codex`
  - 全部 target：`ccb kill`

- **`autonew [target|provider]` / `autonew`** - 重置运行中的会话，不影响无关 target
  - 单 target：`autonew codex@1`
  - 单 provider：`autonew codex`
  - 全部 target：`autonew`

- **`pend <provider> [N]`** - 统一的查看回复命令
  - 当前是 provider 级视图，不是 target 级视图
  - 例：`pend codex 5`
  - 不要写 `pend codex@2`

- **`ccb-mounted [--json|--simple] [--autostart] [path]`** - 查看哪些 target 真的挂载并探活成功
  - `--json` 输出 `mounted` 与 `mounted_providers`
  - `--simple` 输出空格分隔的 mounted target 列表
  - `--autostart` 会先尝试拉起离线 provider daemon
  - 多实例完整说明见 [`TARGET_USAGE_GUIDE.zh.md`](TARGET_USAGE_GUIDE.zh.md)
